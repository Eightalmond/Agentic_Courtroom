...
import { generateJudgeVerdict, validateJudgeEligibility } from "@/lib/courtroom/service";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validated = validateJudgeEligibility(body);
    release = acquireProviderRequest(request, "courtroom", validated.runId);
    if (!release) {
      return NextResponse.json(
        { code: "JUDGE_REQUEST_IN_PROGRESS", message: "A courtroom model request is already running for this test.", retryable: true },
        { status: 409 },
      );
    }
    const execution = await executeRateLimited(request, "courtroom", () => generateJudgeVerdict(body));
