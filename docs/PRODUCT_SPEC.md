...
The timeline records successfully executed customer actions with public explanations, sanitized inputs, deterministic observations, source identifiers, and timestamps. Provider and invalid-tool failures remain safe retry state rather than customer actions. `ANSWER` and `GIVE_UP` each consume one action and complete the run. Reaching the configured successful-action limit completes it as budget exhausted without fabricating an answer. Provider failures and retries do not consume the customer-action budget. This phase records the customer's conclusion but does not judge whether it is correct.
